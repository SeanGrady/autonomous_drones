...
class MissionGenerator:
    ...
    def createMission(self, dict_In):
        ...
        mission['plan'] = [{}]
        mission['plan'][0] = createPlanElement('go', ['home'], 0)
        ...
        if (points):
            ...
            mission['plan'].append(createPlanElement('patrol', list_Points, dict_In['repetition']))
...
dict_Config['width'] = 8      # West in meters (only for box)
dict_Config['rotation'] = 0   # Rotation in degrees around SW corner
dict_Config['radius'] = 10     # Radius in meters (only for circle)
dict_Config['altitude'] = 3    # Altitude in meters
dict_Config['filled'] = True   # T/F for whether area is criss-crossed
dict_Config['loc_start'] = np.array([0, 0]) # Start location that offsets all
                                # points, including home

theGenerator = MissionGenerator()
mission_json = theGenerator.createMission(dict_Config)

with open('auto_gen_mission.json', 'w') as infile:
    infile.write(mission_json)
